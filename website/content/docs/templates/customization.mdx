...
For maximum context injection (includes PRD, patterns, and progress):

```handlebars
{{!-- PRD Context (helps agent understand the bigger picture) --}}
{{#if prdContent}}
## PRD: {{prdName}}
...
## Instructions
1. Study PRD and patterns above for context
...
**Default behavior**: Ralph's built-in OOTB templates now include `{{recentProgress}}` to help agents understand context without receiving full iteration history.

### How It Works

1. After each iteration, Ralph extracts key information to `{{progressFile}}` (default: `.ralph-tui/progress.md`)
