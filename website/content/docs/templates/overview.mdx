...
<Callout type="info">
Both templates include **progress tracking** - agents document learnings in `.ralph-tui/progress.md` which persists across iterations via `{{recentProgress}}` and `{{codebasePatterns}}`.
</Callout>
