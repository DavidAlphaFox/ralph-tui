...
Ralph TUI resolves prompt templates in this order (highest to lowest priority):

| Priority | Source | Description |
|----------|--------|-------------|
| 1 | `--prompt <path>` CLI flag | Explicit path provided at runtime |
| 2 | `prompt_template` in config | Path in project or global config |
| 3 | `.ralph-tui/templates/{tracker}.hbs` | Project-level template (per tracker) |
| 4 | `~/.config/ralph-tui/templates/{tracker}.hbs` | User-level template (per tracker) |
| 5 | Tracker plugin template | Template from tracker's `getTemplate()` method |
| 6 | Built-in default | Bundled fallback template |

<Callout type="tip">
Use `ralph-tui template view` to see which template would be used, and `ralph-tui template create` to scaffold a custom template at the appropriate location.
</Callout>
